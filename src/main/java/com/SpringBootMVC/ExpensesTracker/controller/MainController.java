...
public String list(Model model, HttpSession session) {
    ...
    @GetMapping("/charts")
public String showCharts(HttpSession session, Model model) {
    Client client = (Client) session.getAttribute("client");
    if (client == null) return "redirect:/login";

    int clientId = client.getId();

    int totalExpense = expenseService.getTotalExpenses(clientId);
    model.addAttribute("totalExpense", totalExpense);

    var categoryExpenseMap = expenseService.getCategoryWiseTotalExpenses(clientId);
    model.addAttribute("categoryExpenseMap", categoryExpenseMap);

    var monthlyExpenseMap = expenseService.getMonthlyExpenseTrend(clientId);
    model.addAttribute("monthlyExpenseMap", monthlyExpenseMap);
